'use client'

import { useState } from 'react'
import { useRouter } from 'next/navigation'
import { adminLogin } from '../../../services/adminAuthService'

export default function AdminLoginPage() {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')

  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault()

    setLoading(true)
    setError('')

    const formData = new FormData(e.currentTarget)
    const response = await adminLogin(formData)

    if (response.success) {
      if (response.admin?.role !== 'admin') {
        setError('Ce compte n’est pas un compte administrateur')
        setLoading(false)
        return
      }

      localStorage.clear()

      localStorage.setItem('abg_admin_token', response.token)
      localStorage.setItem('abg_admin', JSON.stringify(response.admin))

      router.push('/admin/dashboard')
    } else {
      setError(response.message || 'Erreur de connexion')
    }

    setLoading(false)
  }

  return (
    <main className="min-h-screen bg-[#071739] flex items-center justify-center px-6">
      <div className="bg-white rounded-[35px] shadow-2xl p-10 w-full max-w-md">
        <h1 className="text-3xl font-extrabold text-[#071739] text-center">
          Administration ABG
        </h1>

        <p className="text-gray-500 text-center mt-3">
          Connectez-vous pour accéder au tableau de bord.
        </p>

        {error && (
          <div className="bg-red-50 text-red-600 p-4 rounded-xl mt-6 text-sm">
            {error}
          </div>
        )}

        <form onSubmit={handleSubmit} className="space-y-5 mt-8">
          <input
            name="email"
            type="email"
            placeholder="Email administrateur"
            className="w-full border border-gray-200 rounded-2xl px-5 py-4 outline-none focus:border-[#D4A63D]"
            required
          />

          <input
            name="password"
            type="password"
            placeholder="Mot de passe"
            className="w-full border border-gray-200 rounded-2xl px-5 py-4 outline-none focus:border-[#D4A63D]"
            required
          />

          <button
            disabled={loading}
            className="w-full bg-[#D4A63D] py-4 rounded-2xl font-bold text-[#071739] hover:scale-[1.02] transition"
          >
            {loading ? 'Connexion...' : 'Se connecter'}
          </button>
        </form>
      </div>
    </main>
  )
}